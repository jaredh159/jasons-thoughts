...
const Chrome: React.FC<Props> = ({ page, smallFooter, children }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen relative">
      <div
        className={cx(
          'z-40 bg-black left-0 top-0 w-screen h-screen xs:hidden bg-opacity-70 fixed',
      ...
      <nav
        className={cx(
          'fixed xs:hidden h-screen w-72 bg-white shadow-xl z-50 left-0 top-0 [transition:150ms] rounded-r-xl overflow-hidden',
          navOpen ? 'ml-0' : '-ml-80',
        )}
      >
        <button
      ...
      <header className="flex flex-row-reverse xs:flex-row justify-between items-center py-5 px-5 sm:px-10 top-0 bg-white bg-opacity-20">
        <Image
          src={ProfilePic.src}
        ...
        />
        <button
          className="xs:hidden border-[0.5px] w-12 h-12 rounded-full shadow-lg flex justify-center items-center transition duration-100 hover:bg-sky-50 text-gray-400 hover:text-gray-500 text-lg"
